...
const getActionIcon = (actionType: string) => {
    switch (actionType) {
        ...
        case 'SUBMITTED':
            return <Send color="info" fontSize="small" />;
        case 'OPENED':
        ...
        case 'RESUBMITTED':
            return <Replay color="primary" fontSize="small" />;
        default:
...
export default function DokumenDiajukanPage() {
    ...
    const fetchDocuments = async () => {
        ...
        try {
            ...
            const submitted = (data.data || []).filter((d: Document) => d.approval_status !== 'DRAFT' && d.approval_status !== 'ARCHIVED');
        ...
        <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                <Box>
                    ...
                    <Typography color="text.secondary">Lacak status dokumen yang sudah Anda ajukan</Typography>
                </Box>
                        ...
                        {documents.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={6} align="center" sx={{ py: 4 }}>
                                    <Typography color="text.secondary">📄 Belum ada dokumen yang diajukan</Typography>
                                </TableCell>
                            </TableRow>
                        ) : (
                            documents.map((doc, index) => (
                                <React.Fragment key={doc.id}>
                                    <TableRow hover>
                                        ...
                                        <TableCell>
                                            <Chip label={STATUS_LABELS[doc.approval_status] || doc.approval_status} color={STATUS_COLORS[doc.approval_status] || 'default'} size="small" />
                                        </TableCell>
                                        <TableCell>
                                            {doc.currentApprover?.full_name || (doc.approval_status === 'SIAP_CETAK' ? 'Selesai' : doc.approval_status === 'SUDAH_DICETAK' ? 'Dicetak' : doc.approval_status === 'DITOLAK' ? 'Ditolak' : '-')}
                                        </TableCell>
                                        <TableCell>{formatDate(doc.created_at)}</TableCell>
                                        <TableCell>
                                            <Box sx={{ display: 'flex', gap: 0.5 }}>
                                                <IconButton size="small" onClick={() => toggleExpand(doc.id)} color={expandedRow === doc.id ? 'primary' : 'default'}>
                                                    {expandedRow === doc.id ? <ExpandLess /> : <ExpandMore />}
                                                </IconButton>
                                                <IconButton size="small" color="primary" href={doc.document_link} target="_blank">
                                                    <OpenInNew />
                                                </IconButton>
                                                <Button size="small" variant="outlined" startIcon={<History />} onClick={() => navigate(`/review-history/${doc.id}`)}>
                                                    Riwayat
                                                </Button>
                                            </Box>
                                        </TableCell>
                                                            ...
                                                            <Card variant="outlined" sx={{ bgcolor: '#e3f2fd', borderColor: 'primary.main' }}>
                                                                <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                                        ...
                                                                        <Chip label="Posisi Saat Ini" color="primary" size="small" />
                                                                    ...
                                                                    <Card key={item.id} variant="outlined" sx={{ bgcolor: 'white' }}>
                                                                        <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                                                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                                                ...
                                                                                <Chip label={ACTION_LABELS[item.action_type] || item.action_type} color={getActionColor(item.action_type)} size="small" variant="outlined" />
                                                                                <Typography variant="body2">
